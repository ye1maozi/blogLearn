...
<script>
import { getSelfPermission, getRolePermissionData, changeRolePermission } from '@/api/menu'
export default {
  name: 'RoleList',
  props: ['show', 'showData'],
  data () {
  ...
  methods: {
    getData () {
      ...
      console.log('[role2permission] getdata start')
      this.modalTree.loading = true
      if (this.getAll === false) {
        getSelfPermission(this.showData.name).then(res => {
          this.getAll = true
      ...
      getRolePermissionData(this.showData.name).then(res => {
    ...
    editMenu () {
      ...
      console.log({ ...param })
      changeRolePermission(param).then(res => {
    ...
    makeSelectedDeptData: function (data, keys, disableCheck) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (oldSelect >= 0 && curSelect >= 0) {
          ...
          this.modalTree.changeMenuData.push({ permissionid: id, newFlag: false })
        } else if (oldSelect < 0 && curSelect >= 0) {
          // 新增的
          // console.log('new flag ', id)
          this.modalTree.changeMenuData.push({ permissionid: id, newFlag: true })
        }
        if (item.children) {
