...
const constantRouterComponents = {
  ...
  'permissionList': () => import('@/views/permission/permissionList')
}
...
export const generator = (routerMap, parent) => {
  console.log('[menu]generator', routerMap)
  return routerMap.map(item => {
    ...
    if (constantRouterComponents[item.component || item.key] != null) {
      ...
      currentRouter.component = () => import(`@/views/${componentPath}`)
    }
    // 是否设置了隐藏菜单
    if (show === false) {
...
const listToTree = (list, tree, parentId) => {
  list.forEach(item => {
    ...
    if (item.parentId == null) {
      ...
      item.meta = {
        title: item.name,
        icon: item.icon ? item.icon : 'dashboard',
        // eslint-disable-next-line eqeqeq
      }

      item.path = '/' + item.path

      item.name = 'async_' + item.name
    }
